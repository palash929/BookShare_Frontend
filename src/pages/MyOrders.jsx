  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const { data } = await axios.get("https://bookshare-backend-ca7c.onrender.com/api/purchases/my", {
  ...
  const downloadInvoice = async (purchaseId) => {
    try {
      ...
      const response = await axios.get(
        `https://bookshare-backend-ca7c.onrender.com/api/purchases/invoice/${purchaseId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
